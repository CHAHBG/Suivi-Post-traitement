(function(){
    ...
    function buildPopover() {
        ...
        pop.style.display = 'none';
            ...
            pop.innerHTML = `
                ...
                <div style="text-align:right; margin-top:0.5rem;"><button id="popoverDetailsBtn" class="action-button">${LOC.details}</button></div>
            `;
    ...
    function openPopover(anchorEl) {
        ...
        if (!fc) content.innerHTML = '<div class="text-xs text-slate-500">Aucune prévision</div>';
        else {
            const achievable = !!fc.achievable;
            const proj = fc.projections || {};
        ...
        try {
            ...
            if (explain) {
                if (!fc) {
                    explain.innerHTML = '<div>Aucune donnée disponible pour expliquer la prévision.</div>';
                } else {
