...
class DataAggregationService {
    ...
    calculateKPIs(rawData) {
        try {
            ...
            try {
                // If we don't have a latestDate (no historical yields present), skip the monthly projection logic safely
                if (!latestDate) {
                    console.warn('Monthly forecast skipped: no latest date available to project from');
                    monthly.forecast = { avgDaily: 0, totalSoFar: 0, projections: {}, achievable: false };
                    monthly.achievable = false;
                    monthly.alert = 'Données insuffisantes pour la prévision mensuelle';
                } else {
